...
usage() {
    cat <<EOF
Generate certificate suitable for admission webhook service.
This script uses k8s' CertificateSigningRequest API to a generate a
certificate signed by k8s CA suitable for admission webhook services.
...
[ -z ${service} ] && service="flink-operator-webhook-service"
[ -z ${secret} ] && secret="webhook-server-cert"
[ -z ${namespace} ] && namespace="flink-operator-system"
